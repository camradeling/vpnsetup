...
# fetches this client's config bundle from the server over SSH (running
# create-client.sh remotely), extracts it into this repo, then installs and
...
Fetches this client's config bundle from a vpnsetup server over SSH
(key-based auth only), installs it into this repo, then runs the
per-protocol client install/configure steps.

Options:
  --server <host>      Server IP or SSH host (required)
...
ssh "${SSH_OPTS[@]}" "$SSH_USER@$SERVER" "bash '$REMOTE_DIR/create-client.sh'" | tar xzf - -C "$REPO_ROOT"
